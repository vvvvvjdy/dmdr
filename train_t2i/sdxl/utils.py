...
def sample_discrete(
    ...
    if s_type == "uniform":
        ...
        return discrete_samples.reshape(B) # Reshape to [B, 1, 1, 1] for broadcasting
        ...
        return discrete_samples.reshape(B)
    else:
...
def sample_continue(
    ...
    if s_type == "uniform":
        ...
        return t.reshape(B) # Reshape to [B, 1, 1, 1] for broadcasting
    elif s_type == "logit_normal":
        if dynamic_step > 0:
            progress = min(step / dynamic_step, 1.0)
            cosine_decay = 0.5 * (1.0 + torch.cos(torch.tensor(progress * 3.141593)))
