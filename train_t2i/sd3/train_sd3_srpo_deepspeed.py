...
from samplers import v2x0_sampler,pred_v
import copy
import math
...
def main(args):
    ...
    if accelerator.mixed_precision == "fp16":
        ...
        # one_dtype only for debug (do not use)
        if args.one_dtype == "fp16":
            train_dtype = torch.float16
            inference_dtype = torch.float16
        elif args.one_dtype == "bf16":
    ...
    pipeline.transformer.requires_grad_(args.use_lora_gen <= 1)
    ...
    # create semantic-style adjectives
    # This is adapted from SRPO (https://github.com/Tencent-Hunyuan/SRPO/blob/main/fastvideo/SRPO.py)
    cg_oily_adjectives = [
        "Painting",
        "CG Render",
        "Oil"
    ...
    realism_adjectives = [
        "Natural-lighting", "Detailed", "Realistic"
    ]
    len_neg = len(cg_oily_adjectives)
    len_pos = len(realism_adjectives)
    ...
    guidance_model = copy.deepcopy(pipeline.transformer)
    # init lora
    if args.use_lora > 1:
        ...
        guidance_model = get_peft_model(guidance_model, transformer_lora_config)
        if args.use_lora_gen > 1:
            transformer_lora_config_gen = LoraConfig(
                r=args.lora_rank * 2,
                lora_alpha=args.lora_rank * 4,
                init_lora_weights="gaussian",
                target_modules=target_modules,
            )
            pipeline.transformer = get_peft_model(pipeline.transformer, transformer_lora_config_gen)
    ...
    if args.resume_ckpt is not None:
        ...
        if accelerator.is_main_process:
            logger.info(f"Resumed from checkpoint: {args.resume_ckpt} at epoch {epoch_start}, global step {global_step}")
    ...
    with torch.no_grad():
        ...
        if global_step == 0:
            with accelerator.autocast():
                with pipeline.transformer.disable_adapter() if args.use_lora_gen > 1 else torch.no_grad():
                    images = pipeline(
                        ...
                        negative_pooled_prompt_embeds=sample_neg_pooled_prompt_embeds[: pooled_prompt_embeds_test.shape[0]],
        ...
        for prompts, metap in train_dataloader:
            ...
            with accelerator.accumulate([gen_model, guidance_model]):
                ...
                with accelerator2.autocast():
                    v_pred_fake = guidance_model(
                        hidden_states=input_latent_gui,
                        timestep=(ts_gui.flatten()* 1000).round().to(device=device, dtype=dtype),
                        encoder_hidden_states=prompt_embeds,
                        pooled_projections=pooled_prompt_embeds,
                        joint_attention_kwargs=scale_dict_f,
                        return_dict=False,
                    )[0]
                ...
                if (inner_step % args.ratio_update == 0) and inner_step > 0:
                    ...
                    # generator update
                    with accelerator.autocast():
                        v_pred_gen = gen_model(
                            hidden_states=input_latent_gen,
                            timestep=(ts.flatten() * 1000).round().to(device=device, dtype=dtype),
                            encoder_hidden_states=prompt_embeds,
                            pooled_projections=pooled_prompt_embeds,
                            joint_attention_kwargs=pipeline.joint_attention_kwargs,
                            return_dict=False,
                        )[0]
                    x0 = input_latent_gen + (0.001 - ts) * v_pred_gen
                    ...
                    if global_step >= args.cold_start_iter:
                        ...
                        if args.hpsv2_reward_model is not None:
                            with accelerator.autocast():
                                ...
                                index_neg = random.choice(range(len_neg))
                                index_pos = random.choice(range(len_pos))
                                prompts_hps_neg = [cg_oily_adjectives[index_neg] + '. ' + prompts[kj] for kj in range(len(prompts))]
                                prompts_hps_pos = [realism_adjectives[index_pos] + '. ' + prompts[kj] for kj in range(len(prompts))]

                                hps2_text_feat_neg = hps2_text_processor(
                                    prompts_hps_neg,
                                    padding="max_length",
                                    max_length=77,
                                    truncation=True,
                                ...
                                hps2_text_feat_pos = hps2_text_processor(
                                    prompts_hps_pos,
                                    padding="max_length",
                                    max_length=77,
                                    truncation=True,
                                ...
                                hps2_img_feat = hps2_model.encode_image(hps2_img_feat["pixel_values"], normalize=True)
                                hps2_text_feat_neg = hps2_model.encode_text(hps2_text_feat_neg.input_ids, normalize=True)
                                hps2_text_feat_pos = hps2_model.encode_text(hps2_text_feat_pos.input_ids, normalize=True)

                                srpo_text_feat =  (args.srpo_k + 1) * hps2_text_feat_pos - hps2_text_feat_neg
                                logits_per_image_hps_srpo = hps2_img_feat @ srpo_text_feat.T

                                reward_loss_hps =  torch.diagonal(logits_per_image_hps_srpo)
                    ...
                    with accelerator.autocast():
                        with torch.no_grad():
                            v_pred_fake = pred_v(
                                transformer=guidance_model,
                                noisy_latents=input_latent_dmd,
                                timesteps=(ts_dmd.flatten() * 1000).round().to(device=device, dtype=dtype),
                                prompt_embeds=prompt_embeds,
                                pooled_prompt_embeds=pooled_prompt_embeds,
                                negative_prompt_embeds=sample_neg_prompt_embeds,
                                negative_pooled_prompt_embeds= sample_neg_pooled_prompt_embeds,
                            ...
                            v_pred_real = pred_v(
                                ...
                                joint_attention_kwargs=scale_dict_r,
                            )
                        x0_r = input_latent_dmd + (0.001 - ts_dmd) * v_pred_real
                        x0_f = input_latent_dmd + (0.001 - ts_dmd) * v_pred_fake
                        p_real = x0 - x0_r
                        p_fake = x0 - x0_f
