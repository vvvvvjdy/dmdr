...
class LabelEmbedder(nn.Module):
    ...
    def forward(self, labels, train, force_drop_ids=None,no_drop=True):
        if no_drop:
            # If no dropout is needed, return embeddings directly
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if use_lora:
            self.lora = QVLoRAUnits(dim,dim,lora_rank)
        else:
            self.lora = None
    ...
    def forward(
        ...
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads * self.head_dim).permute(2, 0, 1, 3).contiguous()
        q, k, v = qkv.unbind(0)
...
class SiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, use_lora=False, lora_rank=0,**block_kwargs):
        super().__init__()
        self.norm1 = nn.LayerNorm(hidden_size, elementwise_affine=False, eps=1e-6)
        self.attn = Attention(
            hidden_size, num_heads=num_heads, qkv_bias=True, qk_norm=block_kwargs["qk_norm"],use_lora=use_lora,lora_rank=lora_rank
        )
    ...
    def forward(self, x, c,lora_scale=1.0):
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
            self.adaLN_modulation(c).chunk(6, dim=-1)
        )
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa),lora_scale)
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            SiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, use_lora=use_lora,lora_rank=lora_rank,**block_kwargs) for _ in range(depth)
        ])
    ...
    def initialize_weights(self):
        ...
        #Zero-out lora layers if used
        if self.use_lora:
            for block in self.blocks:
                if block.attn.lora is not None:
                    nn.init.constant_(block.attn.lora.lora_QB, 0)
                    nn.init.constant_(block.attn.lora.lora_VB, 0)
    ...
    def forward(self, x=None, t=None, y=None, lora_scale=0.0,):
        """
        Forward pass of SiT.
        ...
        for i, block in enumerate(self.blocks):
            x = block(x, c, lora_scale=lora_scale)
...
latent_size = resolution // 8  # 32 for 256x256 images
block_kwargs = {"fused_attn": False, "qk_norm": False}

full_ckpt_path = '/mnt/shared-storage-user/jiangdengyang/sit_1step/sitxl_s1_5kdyna_rldinov2_res256_shift1_nocfg/checkpoints/step-20000.pt'
import os
save_dir = os.path.dirname(full_ckpt_path)
save_name = "beforerl_sitxl_s1_5kdyna_res256_nocfg.bin"
...
model.eval()
state_dict = torch.load(full_ckpt_path, map_location='cpu', weights_only=False)["model"]
model.load_state_dict(state_dict, strict=True)

torch.save(model.state_dict(), save_path)
print(f"Model weights saved to {save_path}")
# reload to check
model_re = SiT_models["SiT-XL/2"](
        input_size=latent_size,
        num_classes=1000,
        use_cfg=2,
        **block_kwargs
...
model_re.eval()
state_dict_re = torch.load(save_path, map_location='cpu', weights_only=False)
model_re.load_state_dict(state_dict_re, strict=True)
print("Model reloaded successfully, weights are consistent.")